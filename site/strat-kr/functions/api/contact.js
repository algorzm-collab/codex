...
export async function onRequest(context) {
    ...
    // Bots commonly fill this field, while normal visitors never see it.
    ...
    const submission = {
        ...
        service: clean(body.service) || '미선택',
        message: clean(body.message)
    };

    if (!submission.name || !submission.phone || !submission.organization ||
        !submission.email || !submission.message || !isEmail(submission.email) ||
    ...
    const safeSubjectOrganization = sanitizeSubject(submission.organization);
    const safeSubjectName = sanitizeSubject(submission.name);
    ...
    try {
        emailResponse = await fetch(RESEND_ENDPOINT, {
            ...
            body: JSON.stringify({
                ...
                subject: `[STRATEGY 상담] ${safeSubjectOrganization} · ${safeSubjectName}`,
                text: [
                    'STRATEGY 홈페이지 상담 신청',
                    '',
                    `성함: ${submission.name}`,
                    `소속: ${submission.organization}`,
                    `연락처: ${submission.phone}`,
                    `이메일: ${submission.email}`,
                    `관심 서비스: ${submission.service}`,
                    '',
                    '문의 내용',
                    submission.message
                ].join('\n'),
                html: `
                    <h2>STRATEGY 홈페이지 상담 신청</h2>
                    <table style="border-collapse:collapse">
                        ...
                        <tr><th style="text-align:left;padding:6px 12px 6px 0">관심 서비스</th><td>${safe.service}</td></tr>
                    </table>
                    <h3>문의 내용</h3>
